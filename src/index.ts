...
async function generateReportForAllRepos(repoOwner: string, repos: string[]) {
  const periods = {
  ...
  // }
  for (const [label, since] of Object.entries(periods)) {
    const periodMetrics: { [user: string]: UserMetric } = {};
    for (const repoName of repos) {
      const metrics = await aggregateMetrics(repoOwner, repoName, since);
      for (const [user, userMetric] of Object.entries(metrics)) {
        const metric = userMetric as UserMetric; // Typecast userMetric
        if (!periodMetrics[user]) {
          periodMetrics[user] = {
            commits: 0,
            pullRequests: 0,
...
async function sendEmailWithAttachment(subject: string, body: string, attachment: Buffer) {
...
function convertReportToXLS(report: any) {
  const sheetData: any[] = [];

  Object.entries(report).forEach(([period, metrics]: [string, any]) => {
    sheetData.push([period]);
    sheetData.push(['User', 'Commits', 'Pull Requests', 'Reviews', 'Score']);
    Object.entries(metrics).forEach(([user, data]: [string, any]) => {
      sheetData.push([
        user,
        data.commits,
        data.pullRequests,
...
async function generateAndEmailReport() {
  ...
  await sendEmailWithAttachment(
    ...
    "Attached is the GitHub metrics report.",
    xlsBuffer
  );
}
